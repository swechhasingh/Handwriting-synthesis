from flask import request, render_template, flash, redirect, jsonify, make_response, url_for, session
...
def submit_style_data():
    ...
    text = data['text']
    if path == '':
        return jsonify(dict({'redirect':url_for('draw'), 'message':"Please enter some style"}))
    ...
    stroke = path_string_to_stroke(path, str_len=len(list(text)), down_sample=down_sample)
    ...
    if 'id' in session:
        ...
        if request.method == 'POST': 
            ...
            if text == '':
                ...
                return render_template('generate.html', title='Generate', message=message,
                                        text="", org_src=org_src, samples="")
            ...
            print(real_text)
            n_samples = 5
            generate_handwriting(char_seq=text, 
                                text_path=text_path, 
            ...
            return render_template('generate.html', title='Generate', text=text, org_src=org_src, samples=gen_samp)
        elif request.method == 'GET':
            return render_template('generate.html', title='Generate', text="", org_src=org_src, samples="", message="")

    return render_template('generate.html', title='Generate', text="", message="Please go to Write and add some style.")
