...
def compute_unconditional_loss(targets, y_hat, mask, M=20):
    ...
    log_constant = log_mixture_weights - math.log(2 * math.pi) - logstd_1 - logstd_2 - 0.5 * torch.log(1 - rho.pow(2))

    x1 = targets[:, :, 1:2]
    x2 = targets[:, :, 2:]

    X1 = ((x1 - mu_1) / torch.exp(logstd_1)).pow(2)
    X2 = ((x2 - mu_2) / torch.exp(logstd_2)).pow(2)
    X1_X2 = 2 * rho * (x1 - mu_1) * (x2 - mu_2) / (torch.exp(logstd_1) * torch.exp(logstd_2))
