...
def argparser():
    ...
    parser.add_argument('--model_path', type=str,
                        default='./results/best_model_prediction.pt')
...
if __name__ == '__main__':
    ...
    if args.prime and args.file_path:
        style = np.load(args.file_path + 'style.npy', allow_pickle=True, encoding='bytes').astype(np.float32)
    ...
    if model == 'prediction':
        ...
        gen_seq, phi = generate_conditional_sequence(
            ...
            train_dataset.idx_to_char, args.bias, args.prime, style, real_text, args.is_map)
        if args.is_map:
